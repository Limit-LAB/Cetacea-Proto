...
///一切`Cetacea`协议使用`MessagePack`作为应用层（表示层）协议。暂时以`jsonmpk`库作为事实标准。
///
///一切`Cetacea`协议使用`WebSocket`作为传输层协议。
///
///使用`Json`作为协议的人类可读格式，仅在调试和格式转换时使用。`Json`到`MessagePack`的转换行为标准暂时以`jsonmpk`库为事实标准。
///
///## 通用协议
///
///### 消息回复格式
...
#[derive(Serialize)]
pub struct MessageResult<T: Serialize + DeserializeOwned, E: Serialize + DeserializeOwned> (pub Result<T, E>);
...
impl<T: Serialize + DeserializeOwned, E: Serialize + DeserializeOwned> From<MessageResult<T, E>> for Result<MessageResponse, serde_json::Error> {
...
pub enum VendorType {
  ...
  NVIDIA , CSR , Myriad , AMD ,
  Mesa , SUSE , OpenEmbedded,
  OtherVendor(String),
}
...
pub enum OSType {
  ...
  ZOS , Haiku , Minix , RTEMS ,
  NaCl , AIX , CUDA , NVCL ,
  AMDHSA , PS4 , PS5 , ELFIAMCU ,
  TvOS , WatchOS , DriverKit , Mesa3D ,
  Contiki , AMDPAL , HermitCore , Hurd ,
  WASI , Emscripten , ShaderModel , LiteOS ,
  OtherOS(String),
}
